...
<template>
    <div>
        ...
        <el-table :data="tableData" v-loading="tableLoading" style="margin-top: 20px">
            ...
            <el-table-column prop="subtitle" label="副标题" width=200></el-table-column>
            <el-table-column label="点击量" width=300>
    ...
    export default {
        ...
        methods: {
            fetch() {
                ...
                axios({
                    method: 'post',
                    url: '/cms/clickRate/countByCategory',//api/webstart/clickRate/countByCategory
                ...
                .then(function(res){
                    ...
                    res.data.object.forEach(function(item){
                        if(item.name == "banner"){
                            item.name = _that.categoryZhCn[item.name] + item.id;
                            tmpData.push(item);
                        } else if(item.name == "category" || item.name == "recommend" || item.name == "product"){
                            item.name = _that.categoryZhCn[item.name];
                            tmpData.push(item);
                        } else {}              
                    });
